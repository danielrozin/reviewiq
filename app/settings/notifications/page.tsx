    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {!prefs && !error && (
        <div role="status" aria-label="Loading notification preferences" className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div key={i} aria-hidden="true" className="h-28 bg-gray-100 rounded-2xl motion-safe:animate-pulse" />
          ))}
        </div>
      )}
                ...
                <div key={key} className="flex items-center justify-between px-5 py-4 gap-4 hover:bg-gray-50/60 transition-colors">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {saved === key && (
                        <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full motion-safe:animate-fade-in">
