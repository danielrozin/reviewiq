    ...
    <div>
      ...
      <div role="search" className="relative mb-8 max-w-md">
        ...
        <input
          ...
          autoComplete="off"
          className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl text-sm text-gray-900 placeholder:text-gray-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-600 focus-visible:border-transparent bg-white"
        />
        {query && (
          <button
            type="button"
            onClick={() => setQuery("")}
            className="absolute right-3 top-1/2 -translate-y-1/2 p-1 -m-1 text-gray-500 hover:text-gray-700 transition-colors touch-manipulation focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600 focus-visible:ring-offset-1 rounded"
      ...
      <ul role="list" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 list-none p-0 m-0">
        {filtered.map((cat) => (
          ...
          <Link
            ...
            className="group bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg hover:border-brand-200 motion-safe:hover:-translate-y-0.5 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600 focus-visible:ring-offset-2 rounded-2xl"
          >
            {/* Hover accent strip */}
            ...
            <div className="flex items-start justify-between mb-3">
              <span aria-hidden="true" className="text-4xl motion-safe:group-hover:scale-110 transition-transform duration-200 inline-block">
                {cat.icon}
              </span>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 px-6 bg-white border border-gray-100 rounded-2xl">
          ...
          <p className="text-xs text-gray-600 mb-3">Try a different search term</p>
          <button type="button" onClick={() => setQuery("")} className="inline-flex items-center gap-1.5 text-xs font-medium text-brand-600 bg-brand-50 px-3 min-h-[44px] touch-manipulation rounded-xl hover:bg-brand-100 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600 focus-visible:ring-offset-1">
