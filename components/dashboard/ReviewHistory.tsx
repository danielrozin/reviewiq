  ...
  if (reviews.length === 0) {
    return (
      <div className="text-center py-10 px-6 bg-white border border-gray-100 rounded-2xl">
        <div aria-hidden="true" className="w-12 h-12 bg-amber-50 rounded-xl flex items-center justify-center mx-auto mb-3">
          <svg aria-hidden="true" className="w-6 h-6 text-amber-600" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
    ...
    <ul role="list" className="space-y-3 list-none p-0 m-0">
      {reviews.map(({ review, productName, productSlug, categorySlug }) => (
        ...
        <article
          ...
          className="bg-white border border-gray-100 rounded-xl overflow-hidden hover:border-gray-200 hover:shadow-sm motion-safe:hover:-translate-y-0.5 transition-all duration-200 group"
        >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1 min-w-0">
              ...
              <h3 id={`rh-headline-${review.id}`} className="text-sm font-semibold text-gray-900 mt-1 truncate">
                {review.headline}
              </h3>
          ...
          <div className="flex items-center gap-4 mt-3 text-xs text-gray-600">
            <time dateTime={review.createdAt}>
                {new Intl.DateTimeFormat("en-US", { year: "numeric", month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(review.createdAt))}
              </time>
