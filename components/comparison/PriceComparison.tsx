...
function PriceCard({ product, isLowest }: { product: Product; isLowest: boolean }) {
  ...
  return (
    <div className={`border rounded-xl p-5 text-center hover:shadow-sm transition-all ${isLowest ? "border-emerald-200 bg-emerald-50/40" : "border-gray-100 hover:border-gray-200"}`}>
      {isLowest && (
        <span className="inline-flex items-center gap-1 text-[10px] font-bold text-emerald-700 bg-emerald-100 border border-emerald-200 px-2 py-0.5 rounded-full mb-2">
