...
export function SpecsComparisonTable({
  ...
  return (
    <section aria-labelledby="specs-comparison">
          ...
          <tbody>
            {Array.from(groups.entries()).map(([groupName, specs], gi) => (
              <Fragment key={groupName}>
                {groups.size > 1 && (
                  <tr>
                    <th colSpan={3} scope="colgroup" className="px-4 py-2 bg-gray-50/60 text-xs font-semibold text-gray-400 uppercase tracking-widest border-b border-gray-100 text-left font-semibold">
                      {groupName}
                    </th>
                  </tr>
                )}
