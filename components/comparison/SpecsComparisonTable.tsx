          ...
          <tbody>
            {Array.from(groups.entries()).map(([groupName, specs], gi) => (
              <Fragment key={groupName}>
                {groups.size > 1 && (
                  <tr>
                    <th colSpan={3} className="px-4 py-2 bg-gray-50/60 text-xs font-semibold text-gray-600 uppercase tracking-widest border-b border-gray-100 text-left font-semibold">
                      {groupName}
                    </th>
                  </tr>
                )}
