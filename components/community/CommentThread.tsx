      ...
      <div className={`group py-4 px-1 rounded-xl transition-colors hover:bg-gray-50/60 ${depth === 0 ? "border-b border-gray-50" : ""}`}>
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          {comment.isOwnerVerified && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded-full border border-emerald-200">
          ...
          {comment.isTopAnswer && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded-full border border-blue-200">
              <svg aria-hidden="true" className="w-2.5 h-2.5 shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
          ...
          <time dateTime={comment.createdAt} className="text-xs text-gray-500 ml-auto">
            {new Intl.DateTimeFormat("en-US", { year: "numeric", month: "short", day: "numeric", timeZone: "UTC" }).format(new Date(comment.createdAt))}
          </time>
        </div>
        ...
        {/* Comment actions */}
        <div role="group" aria-label="Comment actions" className="flex items-center gap-3">
          <VoteControls
            itemId={comment.id}
            itemType="comment"
          ...
          <div className="flex items-center gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
            <button type="button" className="inline-flex items-center gap-1 text-xs text-gray-600 hover:text-brand-600 transition-colors font-medium min-h-[44px] px-3 py-2 rounded-lg hover:bg-brand-50 touch-manipulation">
              <svg aria-hidden="true" className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 15 3 9m0 0 6-6M3 9h12a6 6 0 0 1 0 12h-3" />
              </svg>
              Reply
            </button>
            <button type="button" className="inline-flex items-center gap-1 text-xs text-gray-600 hover:text-gray-800 transition-colors min-h-[44px] px-3 py-2 rounded-lg hover:bg-gray-100 touch-manipulation">
              <svg aria-hidden="true" className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
